...
static void CreateVertexBuffer()//создание треугольника
{
    Vector3f Vertices[3];
...
static void AddShader(GLuint ShaderProgram, const char* pShaderText, GLenum ShaderType)
{
    ...
    const GLchar* p[1];//исходный код шейдера
    p[0] = pShaderText;
    GLint Lengths[1];
    ...
    GLint success;//получает статус компиляции и выводит ошибки шейдера
...
static void CompileShaders()
{
    ...
    glValidateProgram(ShaderProgram);//проверка возможности запуска програмы на данном этапе конвейера
...
int main(int argc, char** argv)
{
    ...
    glutCreateWindow("Tutorial 08: Scale transformations");

    GlutCallbacks();

    // выполняется после инициализации glut
    GLenum res = glewInit();
    if (res != GLEW_OK) {
